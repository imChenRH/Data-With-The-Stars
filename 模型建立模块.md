...
| 假设编号 | 假设内容 | 合理性说明 |
|---------|---------|-----------|
| **D1** | 随机效应服从正态分布：$b_{\text{Pro}} \sim N(0, \sigma^2_{\text{Pro}})$ | 标准混合效应模型假设，便于参数估计；舞伴效应可视为随机抽取的样本 |
| **D2** | 残差独立同分布：$\epsilon \sim N(0, \sigma^2)$ | 同一选手跨周数据通过随机效应建模关联；残差正态性可通过Q-Q图验证 |
| **D3** | 固定效应与随机效应正交 | 设计矩阵构建时验证；舞伴分配与选手特征（年龄、行业）相互独立 |
| **D4** | 舞伴效应在选手间可交换（exchangeable） | 同一舞伴对不同选手的影响具有一致性，仅量上有差异 |
| **D5** | 缺失数据为随机缺失（MAR） | 部分选手缺失年龄/行业信息，假设缺失机制与未观测值无关，可用多重插补处理 |
...
| 假设编号 | 假设内容 | 合理性说明 |
|---------|---------|-----------|
| **E1** | 特征与目标存在非线性关系 | 树模型自动捕捉非线性和交互效应，无需预设函数形式；年龄与表现的关系可能呈倒U型 |
| **E2** | SHAP值具有加性分解性质 | Shapley值的数学性质保证：$f(x) = \phi_0 + \sum_j \phi_j$，所有特征贡献之和等于预测值 |
| **E3** | 训练数据代表总体分布 | 34季数据覆盖足够时间跨度（2005-2024），包含多种行业、年龄段的名人 |
| **E4** | 特征之间存在一定独立性 | 避免严重多重共线性导致SHAP值不稳定；通过VIF检查并剔除高相关特征 |
| **E5** | 目标变量（存活周数）近似连续 | 存活周数范围1-11周，可视为连续变量进行回归；若分布高度偏斜可考虑对数变换 |
...
| 假设编号 | 假设内容 | 合理性说明 |
|---------|---------|-----------|
| **F1** | 历史数据可作为新系统模拟基础 | 选手特征与投票模式具有时间稳定性；粉丝行为模式在短期内不会剧变 |
| **F2** | 三个目标之间存在权衡关系 | 符合社会选择理论的阿罗不可能定理；公平性与娱乐性天然存在张力 |
| **F3** | 帕累托最优解集代表可行的系统设计空间 | 多目标优化的标准解释框架；非支配解均为合理选择 |
| **F4** | 决策变量连续可调 | 评委/粉丝权重可在[0,1]范围内连续变化；实际实施时可离散化 |
| **F5** | 目标函数可通过历史数据精确计算 | 公平性/稳定性/娱乐性的操作化定义有明确数学表达；问题一的投票估算提供必要输入 |
